...
def test_post_merge_hook_end_to_end_and_chaining(tmp_path: Path) -> None:
    """Verify hooks/post-merge cleans up merged worktrees and chains to repo-local hook."""
    ...
    hook_script = Path(__file__).resolve().parents[1] / "post-merge"
...
def test_post_merge_hook_preserves_dirty_worktree(tmp_path: Path) -> None:
    """Verify hooks/post-merge preserves dirty worktrees."""
    origin = tmp_path / "origin"
    origin.mkdir()
    _git(["init", "--bare", "-b", "main"], cwd=origin)
