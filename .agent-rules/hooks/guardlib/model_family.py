...
# --- Rung (junior/escalation -> model), for SUBAGENT spawns only ------
#
...
# rung (junior/escalation) for that platform? Never merged into inspect()/reason() —
# family mismatch is a hard deny, tier mismatch is a nudge a caller can
# override. See ../../harness/effort-models.json for the data and its unverified
# `effort` fields.

_EFFORT_TABLE_PATH = Path(__file__).resolve().parents[2] / "harness" / "effort-models.json"
...
# (`cursor-grok-4.7-medium`). `-fast` and other product variants are NOT in
...
def matches_tier_model(requested: str, tier_model: str) -> bool:
    ...
    `effort-models.json` keeps short names (`grok-4.7`, `composer-2.5`); live
    Cursor Task spawns often pass `cursor-grok-4.7-medium`. Accept optional
    `cursor-` prefix and an effort/thinking suffix from
    `_TIER_EFFORT_SUFFIXES`. Product variants like `grok-4.7-fast` /
    ...
    if "gemini-" in req:
        if tier == "flash_lite" and ("flash_lite" in req or "flash-lite" in req):
            return True
        if tier == "flash" and "flash" in req and "lite" not in req:
            return True
        if tier == "pro" and "pro" in req and "flash" not in req:
            return True
    return False
...
def tier_table(platform: str) -> str:
    """Formatted junior/escalation -> model summary for nudge messages."""
    tiers = _load_effort_table().get(platform, {})
    parts = []
    for tier in ("junior", "escalation"):
        spec = tiers.get(tier)
        if isinstance(spec, dict) and isinstance(spec.get("model"), str):
