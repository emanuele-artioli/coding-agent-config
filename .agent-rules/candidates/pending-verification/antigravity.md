...
- [x] **Global agents via `~/.gemini/config/agents/` (added 2026-09-19, verified 2026-09-20).**
  Antigravity discovers subagents globally from `~/.gemini/config/agents/*.md` when files
  carry `subagent: true` and in-family model/effort rather than symlinking Claude `model: opus`.
  Generated by `install.py` with ownership markers, digest checks, and in-family Flash rungs.
